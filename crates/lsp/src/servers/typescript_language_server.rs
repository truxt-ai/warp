...
impl LanguageServerCandidate for TypeScriptLanguageServerCandidate {
    ...
    async fn install(
        ...
        anyhow::bail!("install not yet implemented for typescript-language-server")
    }

    async fn fetch_latest_server_metadata(&self) -> anyhow::Result<LanguageServerMetadata> {
        anyhow::bail!("fetch_latest_server_metadata not yet implemented for typescript-language-server")
    }
}
